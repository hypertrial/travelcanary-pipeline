...
3. **Land and finalize.** Official dlt resources append a complete candidate batch. A DuckDB transaction then records acceptance and deletes every superseded raw run together. GDELT streams its ZIP member through a disk-backed CSV stage and performs one native `INSERT OR REPLACE ... SELECT FROM read_csv` upsert inside the same accept/reject transaction, then prunes its rolling window and records acceptance. Writable DuckDB sessions disable `preserve_insertion_order`; optional `DUCKDB_MEMORY_LIMIT` caps session memory.
