...
After a successful dbt build, export every public mart to Parquet for portable
analysis outside the repository tooling:
...
After `make demo`, export the disposable demo warehouse with
`uv run make export-demo-marts` (not bare `export-marts`, which targets the
operator/`DUCKDB_PATH` warehouse).
...
`v*` tag, or rebuild locally with `make demo` followed by `make export-demo-marts`.
